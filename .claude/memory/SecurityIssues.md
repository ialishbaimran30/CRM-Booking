...
**Update 2026-08-27 (full re-audit, same day):** A full re-audit re-verified every fixed finding against the current code (all confirmed intact, 68/68 tests passing) and live-tested several controls rather than only reading code. Two new findings surfaced and are now tracked: **H-6** (IP throttling trusts a spoofable `X-Forwarded-For` header — bypasses every anonymous rate limit in the app, live-verified) and **M-7** (MFA re-enrollment needs no proof of prior possession — a stolen session can silently replace a staff member's second factor, live-verified). `L-4`'s entry was updated with live-verification evidence and its now-larger blast radius. A new informational entry, **I-7**, records the deliberate account-enumeration trade-off from the Login-page OTP fix (accepted risk, not a defect).

**Update 2026-08-28:** **H-6 and M-7 are both FIXED** — see each entry for the live-verified fix and new tests (15 new, 83/83 total passing). H-6: `TRUSTED_PROXY_COUNT`/`NUM_PROXIES` now trust only the configured hop count. M-7: re-authentication is now required to replace a confirmed MFA device, and MFA was additionally extended to Django's own `/admin/` site (clarified scope — not a second CRM Admin seat, which remains correctly forbidden by `accounts_single_admin_seat`). Every HIGH and MEDIUM finding in this file now has a non-OPEN status.
...
**Status: FIXED 2026-08-28** — added `settings.TRUSTED_PROXY_COUNT` (env-configurable, default `1` for a bare Azure App Service front-end), wired into `REST_FRAMEWORK["NUM_PROXIES"]` so DRF's throttle identity now trusts only that many trailing hops of `X-Forwarded-For` instead of the raw client-suppliable string. `core/audit.py::get_client_ip` (used by security logging and the failed-login alert's "ip" scope) was fixed the same way, sharing the same setting, so the two never disagree. **Live re-verified**: 6 login attempts with the same trusted trailing hop but a different spoofed prefix each time now throttle correctly (`401×5` then `429`); the existing 5-failures-in-300s admin alert (`core/alerting.py`, untouched) still fires correctly regardless of header spoofing, since it's keyed by submitted identifier as well as IP. 3 new tests in `accounts/tests.py::LoginThrottleBypassTests`. **Caveat unchanged from discovery**: correctness depends on `TRUSTED_PROXY_COUNT` actually matching Azure's real hop count — confirm against the live deployment (still listed under *Areas needing manual testing*), and note setting it *higher* than the true hop count is actively dangerous (attacker header-padding), so the safe default is to under-set rather than over-set it.
...
**Status: FIXED 2026-08-28** — two parts, both live-verified:
1. **Re-auth before replacing a confirmed device.** `accounts/mfa.py::verify_reauth` + `accounts/views.py::MFASetupView` now require the current password, a valid code from the *existing* device, or a recovery code before `/mfa/setup/` will replace an already-confirmed `TOTPDevice`; first-time enrollment (no existing confirmed device) is unaffected, since there's nothing prior to prove possession of. Re-ran the exact live exploit from the audit: attacker session with no re-auth now gets `403` and the victim's device is untouched; the same session succeeds once given the correct password. 7 new tests in `accounts/tests.py::MFAReenrollmentReauthTests`.
2. **MFA extended to Django's own `/admin/` site.** Per explicit clarification: "two Admin accounts/panels" meant Django's built-in `/admin/` (reachable by anyone with `is_superuser=True`, previously zero MFA) and the CRM's own single Admin role (already MFA-protected via `F-2`) — not two simultaneous CRM Admin role-holders, which the `accounts_single_admin_seat` DB constraint correctly continues to forbid (untouched, per `H-3`/`F-4`). `accounts/admin_forms.py::MFAAdminAuthenticationForm` (wired via `accounts/apps.py::AccountsConfig.ready()`) requires a valid code from the *same* `TOTPDevice` a user may have enrolled through the CRM's own flow before Django's admin login succeeds; `templates/admin/login.html` overrides only enough of Django's stock template to render the added field. Live-verified: enrolled superuser is rejected with no code and with a wrong code, succeeds with the correct code or a recovery code; a superuser who never enrolled MFA is completely unaffected. 6 new tests in `accounts/tests.py::DjangoAdminMFATests`.

Neither change touches `TeamRoleAssignment`, the single-Admin-seat constraint, or `F-4`'s break-glass command — no new public admin-recovery endpoint was added. 83/83 backend tests passing.
...
**Update 2026-08-27:** all of the above are done.

**Update 2026-08-28:** H-6 and M-7 are also done. What's left: **L-5** (shared cache — makes H-5/H-6/F-6/F-9 accurate cluster-wide instead of per-process), then the rest of the LOW band.
